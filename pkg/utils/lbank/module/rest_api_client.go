...
import (
	...
	"io/ioutil"
	"github.com/EZChain-core/price-service/pkg/utils/lbank/utils"
	"math/rand"
	"net/http"
	"net/url"
	"strconv"
...
//获取新客户端
func NewLbankClientWithKey(apiKey string, secretKey string) *Client {
	var client Client
	config := constant.InitConfigWithSecret(apiKey, secretKey)
	client.Config = *config
	timeout := config.TimeoutSecond
	if timeout <= 0 {
		timeout = 30
